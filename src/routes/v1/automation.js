...
/**
 * @swagger
 * /api/v1/automation/cliente/{id}:
 *   get:
 *     summary: Obtém informações simplificadas de um cliente
 *     tags: [Automation]
 *     parameters:
 *       - in: path
 *         name: id
 ...
 *         schema:
 *           type: string
 *         description: ID do cliente no Monday.com
 *     responses:
 *       200:
...
router.get('/cliente/:id', 
  validateQueryParams(['id']),
  asyncErrorHandler(async (req, res) => {
    const result = await automationController.getClientInfo(req, res);
    return result;
  })
);
