...
/**
 * Script para testar a automação completa com IDs reais do Monday.com
 * Busca itens existentes no board e testa todas as funcionalidades
 */
async function testarAutomacaoComIdsReais() {
  try {
    logger.logger.info('🚀 Iniciando teste automatizado da automação completa com IDs reais');
    ...
    // Busca itens reais do board
    logger.logger.info('🔍 Buscando itens reais do board...');
    ...
    for (let i = 0; i < dadosTeste.length; i++) {
      ...
      try {
        ...
        // Etapa 2: Buscar item no Monday.com (já temos os dados, mas vamos testar a busca)
        logger.logger.info('🔍 Testando busca de item no Monday.com...');
        try {
          const itemEncontrado = await mondayClient.buscarItemPorId(farmacia.id);
          logger.logger.info(`✅ Item encontrado: ${itemEncontrado.elemento}`);
        } catch (error) {
          logger.logger.info(`⚠️ Erro na busca: ${error.message}`);
        }
        
        // Etapa 3: Atribuir responsável
        ...
        const observacao = `🤖 Teste de automação executado em ${new Date().toLocaleString('pt-BR')} - Pasta criada automaticamente para ${farmacia.produto}`;
        ...
        if (i < dadosTeste.length - 1) {
          logger.logger.info('⏳ Aguardando 2 segundos antes do próximo teste...');
          await new Promise(resolve => setTimeout(resolve, 2000));
        }
    ...
    logger.logger.info('\n🏁 Teste automatizado com IDs reais concluído!');
    logger.logSucesso({ 
      operacao: 'Teste de automação completa com IDs reais', 
      resultado: `Testados ${dadosTeste.length} itens reais do board` 
    });
    
  } catch (error) {
    logger.logErro('Teste de automação completa com IDs reais', error);
  }
}

// Executa a função de teste
testarAutomacaoComIdsReais().catch(error => {
  logger.logErro('Execução do teste automatizado com IDs reais', error);
  process.exit(1);
});